...
def find_cars(img, ystart, ystop, scale, svc, X_scaler, orient, pix_per_cell, cell_per_block, spatial_size, hist_bins,
              cspace, hog_channel, spatial_feat=True, hist_feat=True, hog_feat=True):
    ...
    for xb in range(nxsteps):
        for yb in range(nysteps):
            ...
            # test_features = np.concatenate(window_features)
            # print('The find car feature vector length: %s' %len(test_features))
...
def apply_threshold(heatmap, threshold, pre_lable=None):
    # Zero out pixels below the threshold
    adjust = int(threshold/3)
    if pre_lable == None:
        heatmap[heatmap <= threshold] = 0
    else:
        heatmap[pre_lable[0] > 0] += adjust
        if pre_lable[0].min() == 0:
            heatmap[heatmap + adjust <= threshold] = 0
        else:
            heatmap[heatmap <= threshold] = 0
